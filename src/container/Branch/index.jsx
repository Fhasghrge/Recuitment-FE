...
const Branch = () => {
  ...
  const introdInfos = {
    pm: `
            ...
            `,
    fe:
      ` 你可曾思考过，日常浏览的网页，手机上的小程序，是如何呈现到你面前的？
        你是否想过拥有一个自己的个人主页？
        Web前端开发是从网页制作演变而来的，名称上有很明显的时代特征，随着互联网发展，网页不再只是承载单一的文字和图片，各种丰富媒体让网页的内容更加生动，网页上软件化的交互形式为用户提供了更好的使用体验，这些都是基于前端技术实现的。
        加入StarStudio前端组，会使你能够运用常用的前端技术，包括JavaScript、CSS、HTML等传统技术与概念性较强的交互式设计，和艺术性较强的视觉设计等等。`,
    design: `
    对美的追求是艺术行无止境的不竭动力，比起美仑美奂的设计让你眼花缭乱，
    创造美或许更能满足你的欲望，运用手中的软件，
    创造你想要的尤物，星辰工作室设计组能满足你的愿望！
    ...
    `,
    be:
      `后台的小星星，日常必不可缺少的是各种后台变成语言（PHP,java等），结合数据库、服务器等共同组建一个功能完善的web服务器端，实现数据的处理、分析与下发，性能的优化等工作。
        只要你有学习的热情和团队合作，欢迎加入我们!`,
    devops:
      `合格运维的日常是进行工作室服务器、各种线上服务的基础运维。
      我们也将进行服务发布、更新和维护。Starstudio DevOps期待你的加入!`,
    android:
      `Android开发，我们需要你有良好的java和Android开发基础，能够熟练的使用git和数据库，能够在开发过程中善于和其他方向交流，对待问题能够有自己的思考和创造。
      我们更希望你能够有学习新事物的能力和决心`,
    ios: `你有用过苹果的设备么？
                你喜欢苹果的设计理念么？
  ...
  return (
    <div className="branch">
      ...
      <div className="branch-introd" 
            dangerouslySetInnerHTML={{__html:introdInfos[current].replace(/[\r\n]/g, '<br/>')}}></div>
    </div>
  );
};
